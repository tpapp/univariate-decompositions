...
Return (values v w e)."
  (let+ ((vectors (aprog1 (coerce vectors 'vector)
                    ...
                    (assert (every (curry #'length= (length (aref it 0)))
                                   (subseq it 1)))))
         (trends (aops:each trend vectors))
         ((&flet v- (vector1 vector2)
            (aops:each* element-type #'- vector1 vector2)))
         ((&flet f-on-vectors (vectors)
            (aops:margin* element-type function
                          (aops:combine vectors element-type) 0)))
         (f-trend (f-on-vectors trends))
         ((&flet f-deviation (vectors)
            (v- (f-on-vectors vectors) f-trend)))
         (lhs (f-deviation vectors))
         (rhs (aops:generate (lambda (vector-index)
                             ...
                             vectors
                             :position))
         (rhs-sums (aops:margin* element-type (curry #'reduce #'+)
                                 (aops:combine rhs element-type) 0))
...
(deftest test1 (tests)
  (let+ (((&flet g (vector)
            (aops:recycle (/ (reduce #'+ vector) (length vector))
                          :outer vector)))
         (a #(0 1 2))                   ; level: 1
         (b #(3 5 7))                   ; level: 5
         ((&flet f (v)
    ...
    (assert-equalp #(#(-3 0 3) #(-16 0 24)) rhs)
    (assert-equalp #(0 0 0) rhs-error)))

(deftest test2 (tests)
  (let+ (((&flet g (vector)
            (aops:recycle 2 :outer vector)))
         ((&flet f (v)
            (* (aref v 0) (aref v 1))))
         ((&values lhs rhs rhs-error)
